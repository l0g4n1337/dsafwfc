...
class Misc(commands.Cog):
    ...
    @commands.slash_command(description=f"{desc_prefix}Visualiza informazioni su di me.")
    async def about(
            self,
        ...
        if not self.source_owner:
            self.source_owner = await self.bot.get_or_fetch_user(562539445061025802)
    ...
    @commands.command(name="invite", aliases=["convidar"], description="Mostra il mio link di invito per aggiungermi al tuo server.")
