...
def spotify_client(config: dict) -> Optional[Client]:
    ...
    if not config['SPOTIFY_CLIENT_ID']:
        print(f"[IGNORED] - Supporto Spotify: SPOTIFY_CLIENT_ID non impostato nell'host ENV (o file .env).\n{'-'*30}")
        return

    if not config['SPOTIFY_CLIENT_SECRET']:
        print(F"[IGNORED] - Supporto Spotify: SPOTIFY_CLIENT_SECRET non impostato nell'host ENV (o file .env).\n{'-'*30}")
        return
